            ...
            if user_guess[index] not in corr_ex_chars:
                ...
                while i in range(len(dataset)):
                    if user_guess[index] in dataset[i]:
                        ...
                        i += 1
    try:
       del dataset[dataset.index(user_guess)]
    except:
        pass
    def myFunc(word):
        return len(set(word))
...
database = get_wordle_data_set()
a = len(database)
s = 0
for i in range(a):
    dataset = get_wordle_data_set()
    secret_word = database[i]
    print(i)
    h_cuv_dict = H_cuv(dataset)
    g.write(f"{secret_word} ")
    nr = 1
    user_guess = ""
    while (user_guess != secret_word):
        if user_guess != "":
            correct_index_func(user_guess, secret_word)
            h_cuv_dict = H_cuv(dataset)
            nr += 1
        user_guess = h_cuv_dict[0][0]
        g.write(f"{user_guess} ")
    else:
        s +=nr
        g.write(f'{nr}\n')
print(s/a)
